...
The [database reference](../Design/database-schema.md) documents current tables, relationships, constraints and the rationale for snapshots, duplicate copies, stakes and ordered trails. [Deployment and database setup](../Design/deployment.md) describes environment configuration, migration order and the separate frontend/backend hosting requirements.
