...
export default function ChallengeCard({ eventId }: ChallengeCardProps) {
    ...
    return (
      <div className="rounded-2xl bg-white p-6 text-center shadow-[0_2px_30px_-8px_rgba(4,54,115,0.2)]">
        {result.alreadyCompleted ? (
          <p className="text-sm text-gray-500">You've already completed this challenge.</p>
        ) : result.correct ? (
          <p className="font-serif text-lg text-green-700">Correct!</p>
        ) : (
          <p className="font-serif text-lg text-red-600">Not quite.</p>
        )}

        <p className="mt-2 text-sm text-gray-600">
          The answer was: <span className="font-semibold">{result.correctAnswer}</span>
        </p>

        {result.cardAwarded && (
          <div
            className="mt-4 rounded-xl px-4 py-3 text-sm font-semibold"
            style={{ background: `${WITS_GOLD}20`, color: WITS_BLUE }}
          >
  ...
  return (
    <div className="rounded-2xl bg-white p-6 shadow-[0_2px_30px_-8px_rgba(4,54,115,0.2)]">
      <h3 className="font-serif text-lg" style={{ color: WITS_BLUE }}>
        {challenge.question_text}
      </h3>

      {challenge.question_type === "multiple_choice" && challenge.options && (
        <div className="mt-4 flex flex-col gap-2">
          {challenge.options.map((option) => (
            <button
              ...
              style={selectedOption === option ? { background: WITS_BLUE } : undefined}
            >
              {option}
            </button>
          ))}
      ...
      {challenge.question_type === "true_false" && (
      ...
      <button
        onClick={() =>
          submit(challenge.question_type === "text" ? textAnswer : selectedOption ?? "")
        }
        disabled={
          submitting ||
          (challenge.question_type === "text" ? textAnswer.trim() === "" : !selectedOption)
        }
