...
export default function AdminDashboardPage() {
  ...
  return <div className="mx-auto max-w-6xl p-5 text-stone-800 sm:p-8 lg:p-10">
    <header className="skeuo-plate-navy p-6 sm:p-8 rounded-2xl">
      <p className="text-xs font-bold uppercase tracking-widest text-[#C9A24B]">Behind the adventure</p>
      <h1 className="mt-3 text-3xl font-black tracking-tight text-white sm:text-4xl">Quest control room</h1>
      <p className="mt-3 max-w-xl leading-7 text-white/80">Welcome back, creator. A hidden story, a clever challenge, a new discovery — what will you bring to campus today?</p>
      <a href="#create-content" className="skeuo-btn-gold mt-5 px-6 py-3 text-sm font-semibold rounded-xl inline-flex items-center focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#C9A24B]">Let&apos;s create something <span aria-hidden="true" className="ml-3">↓</span></a>
    </header>
    <section id="create-content" className="mt-8 scroll-mt-6" aria-labelledby="create-heading">
      ...
      <nav className="mt-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Content management">
        {workspaces.map(workspace => <Link key={workspace.key} href={`/dashboard/admin/${workspace.key}`} className="skeuo-card-interactive p-5 rounded-2xl focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#76561C]">
          <div className="flex items-center justify-between gap-3"><span aria-hidden="true" className="text-2xl">{workspace.icon}</span><span className="skeuo-badge-blue text-[10px]">{loading ? "Loading…" : counts[workspace.key] == null ? "Count unavailable" : `${counts[workspace.key]} saved`}</span></div>
    ...
    {error && <p role="alert" className="mt-4 rounded-xl bg-red-50 p-4 text-sm text-red-800 border border-red-200">Some content could not be loaded. {error}</p>}
    <div className="mt-8 grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_280px]">
      <section className="min-w-0 skeuo-card p-5 sm:p-6 rounded-2xl">
        <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-xl font-bold">On your workbench</h2><Link href="/dashboard/admin/events" className="text-sm font-semibold text-[#C9A24B] underline underline-offset-4">Manage events</Link></div>
        <p className="mt-2 text-sm text-stone-500">Your saved events and where they stand.</p>
        {loading ? <p role="status" className="mt-5 text-sm">Loading your creations…</p> : counts.events == null ? <p className="mt-5 text-sm">Events are unavailable right now. Open the events workspace to try again.</p> : events.length === 0 ? <p className="mt-5 skeuo-well rounded-xl p-4 text-sm">Your next adventure starts here. Create your first event above.</p> :
      ...
      <aside className="skeuo-well p-6 rounded-2xl">
        ...
        ].map(([title, detail], index) => <li key={title} className="flex gap-3"><span className="skeuo-btn-secondary flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-xs font-bold text-[#45603B]">{index + 1}</span><div><h3 className="font-semibold">{title}</h3><p className="mt-1 leading-6 text-stone-600">{detail}</p></div></li>)}</ol>
